...
public class BitmapColumnarFilter
{
    // Generate a BitmapColumnarFilter class per Type to avoid mega-morphic call site when reading a position from input block
    private static final NonEvictableCache<Type, MethodHandle> COLUMNAR_BLOOM_FILTER_CACHE = buildNonEvictableCache(
            CacheBuilder.newBuilder()
                    .maximumSize(100)
                    .expireAfterWrite(2, TimeUnit.HOURS));

    private BitmapColumnarFilter() {}

    public static Supplier<FilterEvaluator> createBitmapFilterEvaluator(BitmapFilter bitmapFilter, int inputChannel)
    {
        return () -> new ColumnarFilterEvaluator(
                new DictionaryAwareColumnarFilter(
                        createColumnarBitmapFilter(bitmapFilter.type(), inputChannel, bitmapFilter).get()));
    }

    private static Supplier<ColumnarFilter> createColumnarBitmapFilter(Type type, int inputChannel, BitmapFilter bitmapFilter)
    {
        ...
        return () -> {
            try {
                ...
                return (ColumnarFilter) filterConstructor.invoke(bitmapFilter, inputChannels);
            }
            catch (Throwable e) {
    ...
    private static MethodHandle generateColumnarBloomFilterClass(Type type)
    {
        ...
        FieldDefinition filterField = classDefinition.declareField(a(PRIVATE, FINAL), "filter", BitmapFilter.class);
        FieldDefinition inputChannelsField = classDefinition.declareField(a(PRIVATE, FINAL), "inputChannels", InputChannels.class);
        Parameter filterParameter = arg("filter", BitmapFilter.class);
        ...
        return constructorMethodHandle(filterClass, BitmapFilter.class, InputChannels.class);
    }
    ...
    private static void generateFilterRangeMethod(ClassDefinition classDefinition)
    {
        ...
        body.append(new ForLoop("nullable range based loop")
                ...
                .body(new BytecodeBlock()
                        .append(generateBitmapFilterTest(scope, block, position, result))
    ...
    private static BytecodeBlock generateBitmapFilterTest(Scope scope, Variable block, Variable position, Variable result)
    {
        BytecodeExpression filter = scope.getThis().getField("filter", BitmapFilter.class);
        // boolean result = this.filter.test(block, position)

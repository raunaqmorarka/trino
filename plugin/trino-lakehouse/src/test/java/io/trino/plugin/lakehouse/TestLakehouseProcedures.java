        ...
        extends AbstractTestQueryFramework
{
    ...
    @Test
    void testExpireSnapshots()
    {
        try (TestTable table = newTrinoTable("test_expire_snapshots", "(id integer)")) {
            assertUpdate("INSERT INTO " + table.getName() + " VALUES 1", 1);
    ...
    {
        try (TestTable table = newTrinoTable("test_remove_orphan_files", "(id integer)")) {
            assertUpdate("INSERT INTO " + table.getName() + " VALUES 1", 1);
    ...
    {
        try (TestTable table = newTrinoTable("test_rollback_to_snapshot", "(id integer)")) {
            assertUpdate("INSERT INTO " + table.getName() + " VALUES 1", 1);
            ...
            assertUpdate("CALL system.rollback_to_snapshot(CURRENT_SCHEMA, '%s', %s)".formatted(table.getName(), snapshotId));
    ...
    void testDropStats()
    {
        try (TestTable table = newTrinoTable("test_drop_stats", "WITH (type = 'HIVE', format = 'PARQUET') AS SELECT 1 id")) {
    ...
    {
        try (TestTable table = newTrinoTable("test_drop_extended_stats", "(id integer)")) {

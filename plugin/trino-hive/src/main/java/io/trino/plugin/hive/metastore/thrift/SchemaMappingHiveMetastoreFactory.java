        ...
        implements HiveMetastoreFactory
{
            ...
            CatalogName catalogName)
    {
        ...
        this.delegatesByPrefix = glueConfig.getSchemaMappingRules()
                .map(rules -> SchemaMappingDelegates.createDelegates(
                        rules,
                        Optional.empty(),
                        glueConfig,
                        fileSystemFactory,
    ...
    {
        HiveMetastore metastore = delegateFactory.createMetastore(identity);
        if (delegatesByPrefix.isEmpty()) {
            return metastore;
        }
        return new SchemaMappingHiveMetastore(metastore, delegatesByPrefix);
    }
}

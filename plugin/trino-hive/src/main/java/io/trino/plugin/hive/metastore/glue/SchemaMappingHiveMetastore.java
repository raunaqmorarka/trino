        ...
        implements HiveMetastore
{
    ...
    private Resolved resolve(String databaseName)
    {
        return delegatesByPrefix.entrySet().stream()
                .filter(entry -> databaseName.startsWith(entry.getKey()))
                .max(Map.Entry.comparingByKey(comparingInt(String::length)))
                .map(entry -> new Resolved(entry.getValue(), databaseName.substring(entry.getKey().length())))
                .orElseGet(() -> new Resolved(defaultDelegate, databaseName));
    }
